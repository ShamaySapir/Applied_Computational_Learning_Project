...
class ImprovedPLSemiSupervised():
    def __init__(self, num_of_total_epoch, dataset_name):
        self.model_name = 'Improved Pseudo-Label Semi-Supervised'
    ...
    def main_process(self, data):
        ...
        self.loss_list.append(self.calculate_loss(labeled_avg_loss, mixed_avg_loss))
        return model, evaluation_params, evaluation_df, np.mean(self.loss_list)

    def train_data_mix(self, X_labeled, X_unlabeled, Y_labeled, Y_unlabeled):
        evaluation_df = DataFrame()
        X_mixed_data, Y_mixed_data = None, None
        # Train the model with only labeled data
        while self.curr_epoch <= self.num_of_total_epoch:
    ...
    def calculate_loss(self, labeled_avg_loss, mixed_avg_loss):
        return np.mean([labeled_avg_loss,mixed_avg_loss])
